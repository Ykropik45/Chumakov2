...
class MyLogTest extends TestCase
{
    ...
    public function testLog(string $str): void {
        MyLog::log($str);
        self::$log[] = $str;
        // сообщает об ошибке, если указанные переменные не имеют один и тот же тип данных и значения
        $this->assertSame(self::$log, MyLog::getLog());
    }

    public function providerLog() {
        return array(
            array('This is line equation'),
            array('This is square Equation')
        );
    }
    ...
    public function testWrite() {
        ...
        MyLog::write();
        // сообщает об ошибке, если указанная папка не существует
        ...
        $textLog=$this->_scandir(
            ...
            'ctime',
            1) [0];
        // сообщает об ошибке, если указанный файл не существует
        $this->assertFileExists("log\\".$textLog);
        // сообщает об ошибке, если файл или папка не доступны для чтения
        $this->assertIsReadable("log\\".$textLog);
        // сообщает об ошибке, если ожидаемый файл не имеет актуального значения
